...
from core.config import _database_url  # noqa: E402
...
def main() -> None:
    ...
    with target_engine.connect() as connection:
        nonempty = [
            ...
            if name in target_meta.tables and connection.execute(select(target_meta.tables[name]).limit(1)).first()
        ]
    if nonempty:
        raise SystemExit(f"Target is not empty ({', '.join(nonempty)}). Import only into a fresh database.")
